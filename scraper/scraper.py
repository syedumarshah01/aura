...
async def extract_product_links(page, subcategory_url):
    ...
    try:
        ...
        # Handle Infinite Scrolling / DOM Lazy-Loading
        ...
        # Return ALL extracted links, completely uncapped
...
async def extract_product_details(context, product_url, subcategory_name, sem):
    async with sem:
        ...
        try:
            await page.route("**/*.{png,jpg,jpeg,svg,gif,webp,css,woff,woff2,ttf}", lambda route: route.abort()) # speed up by blocking media
        ...
        for sub_url in subcategories:
            ...
            for detail in results:
                if detail and detail.get('title'):
                    ...
                    all_products_data.append(detail)
                    
                    # Append newly processed item immediately to raw JSONL data-stream
                    jsonl_path = os.path.join(os.path.dirname(__file__), '..', 'backend', 'data', 'scraped_raw.jsonl')
                    with open(jsonl_path, 'a', encoding='utf-8') as f:
                        f.write(json.dumps(detail) + '\\n')

        print(f"Successfully saved {len(all_products_data)} products.")
